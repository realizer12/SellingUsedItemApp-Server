...
if($map_reason==1){//아이디 찾기

    //핸드폰 번호 중복 되는지 확인하기
    $check_phone_number_used=$pdo->prepare('SELECT phone_num FROM member_info WHERE phone_num=:phone_number');

}else if($map_reason==2){//비밀번호 찾기일때

     //핸드폰 번호 중복 되는지 확인하기
     $check_phone_number_used=$pdo->prepare('SELECT * FROM member_info WHERE phone_num=:phone_number AND email=:login_email');
    //핸드폰 번호  데이터 바인딩
    $check_phone_number_used->bindValue('login_email',$login_email);
...
if($check_phone_number_used){
    ...
    if($map_reason==1){//아이디 찾기일때
        ...
        echo "5";//중복값 없음
            ...
            $result=$check_phone_number_used->fetch();
        
            if($result!=null){//중복값 있음


                if($result['sns_check']>0){//sns 아이디일때
